...
class MambaVisionMixer(nn.Module):
    def __init__(
        ...
        if reorder:
            self.imporantness = nn.Linear(self.d_inner//2, 1, bias=False)
            self.hardsort = False
            self.softsorter = SoftSort(hard=self.hardsort)
    ...
    def forward(self, hidden_states):
        ...
        if self.reorder:
            # print('reordering')
            dt = self.dt_proj(dt)  # Sd @ x
            important_score = rearrange(self.imporantness(dt), "(b l) 1-> b l", l=seqlen)  # K @ Sd @ x
            # values, indices = torch.sort(important_score, dim=-1)
            sort_matrix = self.softsorter(-important_score)  # sort matrix of increasing important_score
            x = torch.einsum('bkl,bdl->bdk', sort_matrix, x)
            x_dbl = self.x_proj(rearrange(x, "b d l -> (b l) d"))
            dt, B, C = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1)
            dt = rearrange(self.dt_proj(dt), "(b l) d -> b d l", l=seqlen)
    ...
    def __init__(self,
        ...
        if conv:
            ...
            self.blocks = nn.ModuleList([Block(dim=dim,
                                               ...
                                               reorder=True if i==0 else False)
                                               for i in range(depth)])
