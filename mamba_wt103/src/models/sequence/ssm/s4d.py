...
class SSKernelDiag(nn.Module):
    ...
    def forward(self, L):
        ...
        # ------------------------------------------EDIT HERE------------------------------------
        temp = torch.exp(dtA) #-1
        #torch.sigmoid(temp.real) + temp.imag*1j
        C = C * (torch.sigmoid(temp.real) + temp.imag*1j - 1) / w
        #torch.sigmoid(torch.exp(K)).real + torch.exp(K).imag*1j
