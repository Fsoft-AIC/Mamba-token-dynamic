    ...
    def experiment(self):
        ...
        if self._experiment is None:
            ...
            if wandb.run is not None:
                ...
                while True:
                    try:
                        dset = str(self._wandb_init['config']['dataset']['_name_'])
                        model = str(self._wandb_init['config']['model']['_name_'])
                        lr = "lr-"+str(self._wandb_init['config']['optimizer']['lr'])
                        decay = "decay-"+ str(self._wandb_init['config']['optimizer']['weight_decay'])
                        if model =="mamba_lm_control" or model =="mamba_lm_control_2":
                            params = "-"+str(self._wandb_init['config']['model']['beta_p']) + "-" +  str(self._wandb_init['config']['model']['K_p']) + "-" +\
                            str(self._wandb_init['config']['model']['K_i'])+"-" +str(self._wandb_init['config']['model']['K_d'])
                            model+=params
                            if self._wandb_init['config']['model']['u_0'] == True:
                                model+= "-u_0"
                            model = model + "_dropout-" + str(self._wandb_init['config']['model']['dropout'])
                        self._wandb_init['name'] = dset + "_" + model
...
class SequenceLightningModule(pl.LightningModule):
    ...
    def on_train_epoch_start(self):
        ...
        os.environ['CURRENT_EPOCH'] = str(self.current_epoch)
...
def train(config):
    os.environ['MAX_EPOCH'] = str(config.trainer.max_epochs)
